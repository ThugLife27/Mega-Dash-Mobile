...
if(Update == 1)
{
	...
	with(oStoreSprite)
	{instance_destroy();}
	if(instance_exists(oRedeemButton2))
		{
			with(oRedeemButton2)
			{instance_destroy();}
		}
	if(instance_exists(oStoreInfoButton))
		{
	...
	else if(Redeem == 1)
	{
		...
		{
			case "Common":
			PoolSelect = CommonAugmentPool;
			break;
			
			case "Rare":
			PoolSelect = RareAugmentPool;
			break;
			
			case "Legendary":
			PoolSelect = LegendaryAugmentPool;
			break;
		}
		
		for(var i = 0; i < 3; i+=1)
		{
			if(array_length(PoolSelect) > 0)
			{
				PoolIndex = irandom(array_length(PoolSelect)-1)
				AugmentChoices[i] = PoolSelect[PoolIndex];
				array_delete(PoolSelect,PoolIndex,1);
			}
		}
		
		for (var i = 0; i < 3; i += 1)
		{
		    if(i < array_length(AugmentChoices))
			{
				instance_create_layer(695+425*i,721,"Instances",oRedeemButton2,
				{
					Augment : AugmentChoices[i],
