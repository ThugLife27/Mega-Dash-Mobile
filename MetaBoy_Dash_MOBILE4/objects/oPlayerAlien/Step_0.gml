...
if(global.AlienTimer >= (15*60) && global.gamestart == 1)
{
	//key_ghost = max(keyboard_check_pressed(vk_enter),gamepad_button_check_pressed(0,gp_face2));
	if(key_UFO == 1)
	{
		global.AlienUFOActive = 1;
		instance_create_layer(-400,500,"Laser",oUFOEntry);
		audio_play_sound(SoundUFOEnter,10,false,1.2);
		oButtonUFO.flash = 20;
		time = 70; //milliseconds
		amplitude = 120; //255 is the max
		androidVibrate(time,amplitude);
	}
}
...
else if (x < 400 && global.dash_mode == 0)
	{
