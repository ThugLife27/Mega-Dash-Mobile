...
if(global.GhostActive == 1 && global.GhostReadyTimer == 0 && !place_meeting(x,y,oWallTutorial))
{global.GhostActive = 0;}

if(global.NinjaActive == 1 && global.NinjaTimer == 0 && !place_meeting(x,y,oWallTutorial))
{
	global.NinjaActive = 0;
	global.NinjaTimer = 60;	
}

if (place_meeting(x+1,y,oWallTutorial) && global.uDemolisher == 0 && global.uHulk == 0 && global.GhostActive == 0 && global.NinjaActive == 0 && global.player_revive_mode == 0)
	{
...
if (place_meeting(x+1,y,oWallTutorial) && global.UltimateActive == 0 && ((global.uDemolisher == 0 && global.uHulk == 0) || global.dash_mode == 0 || hsp < 0) && global.GhostActive == 0 && global.NinjaActive == 0 && global.player_revive_mode == 0)
	{
	while(place_meeting(x,y,oWallTutorial)){x--;}
...
//Vertical Collision
	...
	{
	if (place_meeting(x,y+vsp,oWallTutorial) && global.UltimateActive == 0)
	{
		while(!place_meeting(x,y+sign(vsp),oWallTutorial))
		{
			y = y + sign(vsp);
		}
	...
	//Animation
	if (!place_meeting(x,y+1,oWallTutorial) && !place_meeting(x,y+1,oWallUlt) && global.dash_mode == 0)
	{
		sprite_index = global.CharacterRunSkin;
		image_speed = 0;
		if (sign(vsp)>0) 
...
if(revive_timer == 0 && !place_meeting(x,y,oWallTutorial)){revive_timer = 180;global.player_revive_mode = 0;}
}
